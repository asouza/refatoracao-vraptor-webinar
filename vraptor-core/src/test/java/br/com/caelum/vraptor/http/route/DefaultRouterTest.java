...
public class DefaultRouterTest {
	...
	public void setup() {
		...
		this.criaDefaultRouteBuilder = new CriaDefaultRouteBuilder(
				new NoTypeFinder(), converters, new ParanamerNameProvider(),
				new JavaEvaluator(new DefaultReflectionProvider()),
				encodingHandler, proxifier);

		router = new DefaultRouter(proxifier, cache, criaDefaultRouteBuilder);
	}
	...
	@Test
	public void shouldThrowMethodNotAllowedExceptionWhenNoRoutesMatchTheURIWithGivenHttpMethod()
			throws Exception {
		Route route = mock(Route.class);
		...
		try {
			...
			assertThat(e.getAllowedMethods(),
					is((Set<HttpMethod>) EnumSet.of(HttpMethod.GET)));
		}
	}

	private DefaultControllerMethod anyControllerMethod()
			throws NoSuchMethodException {
		return new DefaultControllerMethod(
				new DefaultBeanClass(MyController.class),
				MyController.class.getMethod("customizedPath"));
	}
	...
	public void shouldObeyPriorityOfRoutes() throws Exception {
		...
		ControllerMethod found = router.parse("anything", HttpMethod.GET,
				request);
	...
	public void acceptsASingleMappingRule()
		...
		when(route.controllerMethod(request, "/clients/add"))
				.thenReturn(method);

		router.add(route);
		ControllerMethod found = router.parse("/clients/add", HttpMethod.POST,
				request);
	...
	@Test
	public void passesTheWebMethod()
			throws SecurityException, NoSuchMethodException {
	...
	public void acceptsAnHttpMethodLimitedMappingRule()
		...
		new Rules(router) {
			@Override
			public void routes() {
				routeFor("/clients/add").with(HttpMethod.POST)
						.is(SomeController.class).add(null);
			}
		};
		assertThat(router.parse("/clients/add", HttpMethod.POST, request),
				is(VRaptorMatchers.controllerMethod(method("add", Dog.class))));
	}

	private Method method(String methodName, Class<?>... params)
			throws SecurityException, NoSuchMethodException {
		return SomeController.class.getDeclaredMethod(methodName, params);
	}

	@Test
	public void acceptsAnHttpMethodLimitedMappingRuleWithBothMethods()
			throws NoSuchMethodException {
		new Rules(router) {
			@Override
			public void routes() {
				routeFor("/clients/add").with(HttpMethod.POST)
						.with(HttpMethod.GET).is(SomeController.class)
						.add(null);
			}
		};
		...
		assertThat(router.parse("/clients/add", HttpMethod.GET, request),
				is(VRaptorMatchers.controllerMethod(method("add", Dog.class))));
	}

	@Test
	public void shouldReturnTheFirstRouteFound() throws Exception {
		Method method = MyController.class.getDeclaredMethod("listDogs",
				Integer.class);
		registerRulesFor(MyController.class);

		assertEquals("/dogs/1", router.urlFor(MyController.class, method,
				new Object[] { "1" }));
	}
	...
	public void usesAsteriskBothWays() throws NoSuchMethodException {
		...
		assertThat(router.parse(url, HttpMethod.POST, null).getMethod(),
				is(equalTo(method)));
	}

	@Test
	public void shouldCacheInvocationsAfterFirstCall()
			throws NoSuchMethodException {
		registerRulesFor(MyController.class);
		final Method method = MyController.class.getMethod("starPath");
	...
	@Test
	public void canTranslateAInheritedControllerBothWays()
			throws NoSuchMethodException {
		registerRulesFor(MyController.class);
		registerRulesFor(InheritanceExample.class);
		final Method method = MyController.class.getMethod("notAnnotated");
		String url = router.urlFor(InheritanceExample.class, method,
				new Object[] {});
		assertThat(router.parse(url, HttpMethod.POST, null).getMethod(),
				is(equalTo(method)));
	}
	...
	@Test
	public void canTranslateAnnotatedMethodBothWays()
			throws NoSuchMethodException {
		registerRulesFor(MyController.class);
